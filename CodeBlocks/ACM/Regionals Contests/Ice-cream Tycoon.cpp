...
char q[100];
int len;
int sumCost[2000005], sumCount[2000005], flushed[2000005];

int check(int node, int c, int m){
    if(c==0) return 1;
    if(node>=len){
...
void flush(int node, int c, int m){
//    printf("%d %d %d\n", node, c, m);
    if(node>=len){
        sumCount[node]-=c;
        sumCost[node]=sumCount[node]*(node-len);
        return;
    }
    if(c==0) return;
...
int main(){
    ...
    int a,b;
    while(scanf("%s %d %d", q, &a, &b)>0){
        if(strcmp(q, "ARRIVE")==0){
            update(1, 0, len, a, b+1);
        }else{
            if(check(1, a, b)){
                printf("HAPPY\n");
                flush(1, a, b);
